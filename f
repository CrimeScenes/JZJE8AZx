...
local function getBoundingSize(distance)
    local silentSettings = shared.Forbidden.Silent
    local ranges = silentSettings.Ranges
    local sizes = silentSettings.BoundingSizes

    if distance <= ranges.CloseDistance then
        return sizes.CloseSize
    elseif distance <= ranges.MidDistance then
        return sizes.MidSize
    elseif distance <= ranges.FarDistance then
        return sizes.FarSize
    end
    return sizes.FarSize  -- Default to FarSize if out of range
end


local function calculateTargetDistance(targetPosition)
    local playerPosition = Local_Player.Character.HumanoidRootPart.Position
    return (targetPosition - playerPosition).Magnitude
end

-- Update FOV drawing function
local function updateFov()
    ...
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        local targetPosition = TargetPlayer.Character.HumanoidRootPart.Position
        local targetDistance = calculateTargetDistance(targetPosition)
        local currentSize = getBoundingSize(targetDistance) -- Use dynamic size based on range

        -- FOV shape drawing code
        if settings.FovShape == "Square" then
            local halfSize = currentSize / 2
            local corners = {
        ...
        elseif settings.FovShape == "Triangle" then
            local points = {
                V2(Mouse.X, Mouse.Y - currentSize),
                V2(Mouse.X + currentSize * math.sin(math.rad(60)), Mouse.Y + currentSize * math.cos(math.rad(60))),
                V2(Mouse.X - currentSize * math.sin(math.rad(60)), Mouse.Y + currentSize * math.cos(math.rad(60)))
            }
            for i = 1, 3 do
                local line = Drawing.new("Line")
...
local function isPartInFovAndVisible(part)
    ...
    local boundingSize = getBoundingSize(distance) -- Use dynamic size based on range
    return onScreen and distance <= boundingSize
end
